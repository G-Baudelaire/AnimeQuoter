...
@bot.command(name="random", help="Responds with a quote from a random character from a random anime.")
async def random_quote(ctx):
    quote = await dbquery.get_random_quote(client)
    await ctx.send(quote)


@bot.command(name="character", help="Responds with a random quote from a random anime character.")
async def character_quote(ctx, *args: str):
    quote = await dbquery.get_character_quote(client, " ".join(args))
    await ctx.send(quote)


@bot.command(name="anime", help="Responds with a random quote from an anime.")
